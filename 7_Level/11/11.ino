...
{ 
  ...
  {
    for ((right_foot = 180; right_foot >= 170; right_foot--) && (left_foot = 180; left_foot >= 170; left_foot--))         //For loop will be running as long as "right_foot" is greater or equal to 80 (right_foot is decrementing itself and in the beginning is set to 90)
    {
      convert();                                                    //"convert" function is being called
    }
    _delay_ms(20);                                                  //Delay of 20ms

    for ((right_foot = 170; right_foot >= 180; right_foot++) && (left_foot = 170; left_foot >= 180; left_foot++))         //For loop will be running as long as "right_foot" is greater or equal to 90 (right_foot is incrementing itself and in the beginning is set to 80)       
    {
      convert();                                                    //"convert" function is being called
    }
    _delay_ms(20);                                                  //Delay of 20ms
