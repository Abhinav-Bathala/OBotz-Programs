...
{ 
  ...
  while(1)                                                          //Calling "while" loop
  {
    ...
    while(1)                                                          //Calling "while" loop
  {
    for (right_hip = 90; right_hip <= 135; right_hip++)           //For loop will be running as long as "right_foot" is greater or equal to 80 (right_foot is decrementing itself and in the beginning is set to 90)
    {
      convert();                                                    //"convert" function is being called
    }
    _delay_ms(20);                                                  //Delay of 20ms

    for (right_hip = 135; right_hip >= 90; right_hip--)           //For loop will be running as long as "right_foot" is greater or equal to 90 (right_foot is incrementing itself and in the beginning is set to 80)       
    {
      convert();                                                    //"convert" function is being called
    }
    _delay_ms(20);                                                  //Delay of 20ms
