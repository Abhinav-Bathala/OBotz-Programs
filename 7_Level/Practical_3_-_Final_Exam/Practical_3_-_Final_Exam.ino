...
int count_angle, count_angle1, count_angle2, count_angle3;        //Indroducing multiple "count_angle" variables
...
void convert()
{
  count_angle = right_fore + 46;          //Servo Motor Angle Conversion
  count_angle1 = right_hind + 46;         //Servo Motor Angle Conversion 
  count_angle2 = left_fore + 46;          //Servo Motor Angle Conversion 
  count_angle3 = left_hind + 46;          //Servo Motor Angle Conversion 
  _delay_us(1200);                        //Delay for 1.2ms
}

void move_forward()
{
  for (int j = 0; j <= 3; j++)          //Repeat Forward Motion 3 times
  {
    for (int i = 1; i <= 20; i++)       //Move right a little bit
      {
        right_fore++;
        left_hind--;
        convert();
      ...
      for (int i = 1; i <= 10; i++)    //Reudjusting
      {
        left_fore++;
        right_hind--;
        convert();
      ...
      for (int i = 1; i <= 20; i++)   //Move left a little bit
      {
        right_fore--;
        left_hind++;
        convert();
      ...
      for (int i = 1; i <= 10; i++)   //Readjusting
      { 
        left_fore--;
        right_hind++;
        convert();
...
void move_right()                     //Repeat Turning Right Motion 3 times
{ 
  for (int j = 0; j <= 3; j++)
  {
    for (int i = 1; i <= 20; i++)     //Move right a lot
    {
      right_hind++;
      right_fore++;
    }
    for (int i = 1; i <= 10; i++)     //Move left a little bit
    {
      left_hind--;
      left_fore--;
    }
...
void home_position()
{
  right_fore = 90;          //Set servo to starting position
  right_hind = 90;          //Set servo to starting position
  left_fore = 90;           //Set servo to starting position
  left_hind = 90;           //Set servo to starting position
}
